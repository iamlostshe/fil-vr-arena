...
class ArrayTranslationDriver implements TranslationDriver
{
    ...
    /**
     * Assert that a game has an attribute translation stored in the provided language.
     *
    ...
    public function assertModelHasTranslation(Model $model, string $attribute, string $language)
    {
        PHPUnit::assertTrue(
            ...
            "Provided game's {$attribute} was not translated to {$language}"
        );
    }

    /**
     * Assert that the game hasn't got a translation for the specified attribute in the specified language.
     *
     * @param \Illuminate\Database\Eloquent\Model $model
     * @param string $attribute
     * @param string $language
    ...
    public function assertModelNotHasTranslation(Model $model, string $attribute, string $language)
    {
        PHPUnit::assertFalse(
            $this->translationExists($model, $attribute, $language),
            "The provided game has {$attribute} translated to {$language}"
        );
    }

    /**
     * Assert that the game's attribute's translation in the provided language has the provided value.
     *
     * @param \Illuminate\Database\Eloquent\Model $model
     * @param string $attribute
     * @param string $language
    ...
    public function assertModelTranslation(Model $model, string $attribute, string $language, string $expected)
    {
        ...
        PHPUnit::assertEquals(
            $expected,
            $translation,
            "The provided game's {$attribute} was translated as {$translation} instead of the expected {$expected}."
        );
    }
